...
class RslRlVecEnvWrapper(VecEnv):
    ...
    def __init__(self, env: ManagerBasedRLEnv | DirectRLEnv | ManagerBasedSafeRLEnv, clip_actions: float | None = None):
        ...
        """
        print(f"[DEBUG] Vecenv_Wrapper: env: {env}")
        self.DEBUG = False
        # check that input is valid
        if not isinstance(env.unwrapped, ManagerBasedRLEnv) and not isinstance(env.unwrapped, DirectRLEnv) and not isinstance(env.unwrapped, ManagerBasedSafeRLEnv):
            raise ValueError(
        ...
        print(f"[DEBUG] Vecenv_Wrapper: unwrapped: {self.unwrapped}")
        if hasattr(self.unwrapped, "cost_limits"):
            self.cost_limits = self.unwrapped.cost_limits
            print(f"[DEBUG] Vecenv_Wrapper: cost_limits: {self.cost_limits}")
    ...
    def step(self, actions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        if self.DEBUG:
            ...
            print(f"[DEBUG] Vecenv_Wrapper Step: actions shape {actions.shape}")
            print(f"[DEBUG] Vecenv_Wrapper Step: obs shape {obs.shape}, rew shape {rew.shape}, dones shape {dones.shape}")
            print(f"[DEBUG] Vecenv_Wrapper Step: extras keys {list(extras.keys())}")
